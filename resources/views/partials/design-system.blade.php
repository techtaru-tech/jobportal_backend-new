{{--
  The design system, shared by every page this app serves.

  Tokens lifted from admin_panel/tailwind.config.js, which in turn took them
  from the Flutter app (app_colors.dart / app_dimens.dart /
  app_text_styles.dart). One copy on purpose: the admin panel and the public
  site are the same product, and two copies of a brand red is how they stop
  looking like it.

  Tailwind, Alpine and the fonts are all served locally. Without Alpine the
  admin panel has no page at all, so it is not something to leave depending
  on a CDN, and the public site should not be slower than it has to be.
--}}
...
{{--
  Each page supplies its own component script and then Alpine, in that
  order — see partials/alpine.blade.php. Deferred scripts run in document
  order, and Alpine fires `alpine:init` the moment it executes, so anything
  registering a component has to come first or `x-data` resolves to nothing.
--}}
<style>
